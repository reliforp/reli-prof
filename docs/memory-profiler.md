...
It can capture the memory contents of the target from outside the process, and analyzing it with the knowledge of the internal structures of the PHP VM. Then dump them all. So target programs don't need any modifications, don't need to load a specific extension for this.

# Requirements
- FFI
- PHP 8.1 or above for execution
- PHP 7.0+ for targets
- Only NTS target is tested currently
...
This is the global variables table. The same table is referenced from the symbol table field of the root call frame.
