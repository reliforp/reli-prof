...
Source: [`bench/results/jit.csv`](results/jit.csv) and
[`bench/results/jit-summary.txt`](results/jit-summary.txt).
...
Source: [`bench/results/raw.csv`](results/raw.csv),
[`bench/results/long.csv`](results/long.csv).
...
Source: [`bench/results/cpu-rates.csv`](results/cpu-rates.csv).
...
Source: [`bench/results/cpu.csv`](results/cpu.csv).)
...
Source: [`bench/results/cpu-samples.csv`](results/cpu-samples.csv).
...
(`bench/run-realworld-interleaved.sh`) where each round runs
...
[`bench/results/realworld.csv`](results/realworld.csv) for the
raw numbers.)
...
Source: [`bench/results/realworld.csv`](results/realworld.csv).
...
Source: [`bench/results/output-sizes.csv`](results/output-sizes.csv),
generated by [`bench/run-output-sizes.sh`](run-output-sizes.sh).
...
target — `depth-100-1M`
(`bench/scripts/depth.php 100 1000000 50`, ~2 s baseline) — so
...
script. An earlier version of `bench/run.sh` used the wrong
...
[`bench/results/cpu-samples.csv`](results/cpu-samples.csv) and
[`bench/results/cpu-rates.csv`](results/cpu-rates.csv) for
reference.
...
1. **`bench/scripts/depth.php` is monomorphic.** It calls a
...
RUNS=3 bash bench/run.sh > bench/results/raw.csv

# Long suite, sampling/light tools only at ~1-3 s baseline, JIT off (n=10).
RUNS=10 bash bench/run-long.sh > bench/results/long.csv

# JIT-on suite (n=10 light / n=3 heavy). SPX excluded.
RUNS_LIGHT=10 RUNS_HEAVY=3 bash bench/run-jit.sh > bench/results/jit.csv

# External samplers (reli, phpspy) on the long benches, JIT off.
RUNS=3 bash bench/run-external.sh > bench/results/external.csv

# Sampler-side CPU (n=5, depth sweep 10/30/100/200).
RUNS=5 bash bench/run-cpu.sh > bench/results/cpu.csv

# Sample-count verification + phpspy -S/-b 65k variants.
bash bench/run-cpu-samples.sh > bench/results/cpu-samples.csv

# Rate sweep 49-1000 Hz × depth 30/100/200.
bash bench/run-cpu-rates.sh > bench/results/cpu-rates.csv

# Aggregate (medians + multipliers vs baseline):
php -n bench/summarize.php bench/results/raw.csv bench/results/external.csv
php -n bench/summarize.php bench/results/long.csv
php -n bench/summarize.php bench/results/jit.csv
php -n bench/summarize-cpu.php bench/results/cpu.csv
```

Setup notes (host install of profilers because the sandbox
couldn't reach public container registries) are in
[`bench/README.md`](README.md). The Dockerfile in this directory
