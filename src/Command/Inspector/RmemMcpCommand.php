...
- **Nodes reachable only from `objects_store`**: Suspicious but not conclusive. These objects have no reference from the call stack or global scope *that reli can trace* — reli may not track all reference paths (e.g., references held inside C extensions). Check the node's `refcount` via `rmem_node_detail`: if refcount equals the number of parents reli found (typically 1 for objects_store-only), the object is likely part of a reference cycle awaiting PHP's cycle collector. If refcount is higher, there are references reli cannot see.
