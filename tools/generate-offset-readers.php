...
foreach ($versions as $version) {
    ...
    /** @var array<string, array<string, int>> $all_offsets struct => constants */
    $all_offsets = [];
    ...
    foreach ($struct_defs as $class_name => $def) {
        ...
        $all_offsets[$class_name] = $layout_constants;
    ...
    // Emit per-version FastPathReader implementation
    $fp_code = generateFastPathReaderClass($version, $all_offsets);
    file_put_contents("{$version_dir}/FastPathReader.php", $fp_code);
    fprintf(STDERR, "  %s/FastPathReader generated\n", $version);
}
...
/**
 * Generate a version-specific FastPathReader implementation class.
 *
 * Every method body has the primitive read inlined with constant
 * offsets, so the JIT sees instance method → ord() with no
 * intermediate dispatch. Instance methods use DO_UCALL which the
 * tracing JIT specializes to near-direct jumps.
 *
 * @param array<string, array<string, int>> $all_offsets
 */
function generateFastPathReaderClass(string $version, array $all_offsets): string
{
    $ns = "Reli\\Inspector\\MemoryDump\\FastPath\\Generated\\{$version}";
    $zval = $all_offsets['Zval'] ?? [];
    $bucket = $all_offsets['Bucket'] ?? [];
    $arr = $all_offsets['ZendArray'] ?? [];
    $str = $all_offsets['ZendString'] ?? [];
    $obj = $all_offsets['ZendObject'] ?? [];

    $zval_size = $zval['SIZE'] ?? 16;
    $bucket_size = $bucket['SIZE'] ?? 32;
    $arr_size = $arr['SIZE'] ?? 56;
    $str_size = $str['SIZE'] ?? 32;
    $obj_size = $obj['SIZE'] ?? 56;

    // Helper to generate a method that reads from a region
    $methods = [];

    // Zval methods
    $methods[] = genMethod('zvalType', $zval_size, 'u8', $zval['OFFSET_U1_V_TYPE'] ?? 8);
    $methods[] = genMethod('zvalValuePtr', $zval_size, 'ptr', $zval['OFFSET_VALUE_PTR'] ?? 0);
    $methods[] = genMethod('zvalValueLval', $zval_size, 'u64le', $zval['OFFSET_VALUE_LVAL'] ?? 0);
    $methods[] = genMethod('zvalTypeInfo', $zval_size, 'u32le', $zval['OFFSET_U1_TYPE_INFO'] ?? 8);

    // Bucket methods
    $methods[] = genMethod('bucketValType', $bucket_size, 'u8', $bucket['OFFSET_VAL_U1_V_TYPE'] ?? 8);
    $methods[] = genMethod('bucketValValuePtr', $bucket_size, 'ptr', $bucket['OFFSET_VAL_VALUE_PTR'] ?? 0);
    $methods[] = genMethod('bucketKeyPtr', $bucket_size, 'ptr', $bucket['OFFSET_KEY'] ?? 24);
    $methods[] = genMethod('bucketH', $bucket_size, 'u64le', $bucket['OFFSET_H'] ?? 16);
    ...
    $methods[] = genMethod('arrayRefcount', $arr_size, 'u32le', $arr['OFFSET_GC_REFCOUNT'] ?? 0);
    $methods[] = genMethod('arrayTypeInfo', $arr_size, 'u32le', $arr['OFFSET_GC_TYPE_INFO'] ?? 4);

    // ZendString methods
    $methods[] = genMethod('stringLen', $str_size, 'u64le', $str['OFFSET_LEN'] ?? 16);
    $methods[] = genMethod('stringH', $str_size, 'u64le', $str['OFFSET_H'] ?? 8);
    $methods[] = genMethod('stringRefcount', $str_size, 'u32le', $str['OFFSET_GC_REFCOUNT'] ?? 0);
    $methods[] = genMethod('stringTypeInfo', $str_size, 'u32le', $str['OFFSET_GC_TYPE_INFO'] ?? 4);

    // ZendObject methods
    $methods[] = genMethod('objectCe', $obj_size, 'ptr', $obj['OFFSET_CE'] ?? 16);
    ...
    $lines[] = '        private RegionByteProvider $regions,';
    $lines[] = '    ) {';
    $lines[] = '    }';
    $lines[] = '';
...
/**
 * Generate lines for a single FastPathReader method.
 *
 * @return list<string>
 */
function genMethod(string $name, int $struct_size, string $read_type, int $offset): array
{
    $expr = inlineReadExpr($read_type, '$region->bytes', "\$off + {$offset}");
    return [
        '#[\Override]',
        "public function {$name}(int \$address): ?int",
        '{',
        "    \$region = \$this->regions->regionFor(\$address, {$struct_size});",
        '    if ($region === null) {',
        '        return null;',
        '    }',
